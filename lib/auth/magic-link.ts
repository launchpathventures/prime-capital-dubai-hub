 ...
 * Output: https://<ref>.supabase.co/auth/v1/verify?token=xxx&type=magiclink&redirect_to=http://localhost:3002/api/auth/callback?next=/learn
 */
export function rewriteMagicLink(supabaseLink: string, requestUrl: string): string {
  const url = new URL(supabaseLink)
  const { origin } = new URL(requestUrl)

  // Build our callback URL with the /learn redirect
