/**
 * CATALYST - Dropdown Menu Component
 *
 * @source shadcn/ui v3.6.2 + @base-ui/react v1.0.0
 * @customised Yes — cursor-pointer on menu items (shadcn uses cursor-default)
 *
 * @usage DropdownMenuLabel must be inside a DropdownMenuGroup:
 *   <DropdownMenuContent>
 *     <DropdownMenuGroup>
    ...
    <MenuPrimitive.Portal>
      ...
      >
        <MenuPrimitive.Popup
          ...
          className={cn("ui-dropdown-menu__content", "data-open:animate-in data-closed:animate-out data-closed:fade-out-0 data-open:fade-in-0 data-closed:zoom-out-95 data-open:zoom-in-95 data-[side=bottom]:slide-in-from-top-2 data-[side=left]:slide-in-from-right-2 data-[side=right]:slide-in-from-left-2 data-[side=top]:slide-in-from-bottom-2 ring-foreground/10 bg-popover text-popover-foreground min-w-32 rounded-md p-1 shadow-md ring-1 duration-100 z-50 max-h-(--available-height) w-(--anchor-width) origin-(--transform-origin) overflow-x-hidden overflow-y-auto outline-none data-closed:overflow-hidden", className )}
          {...props}
        />
      </MenuPrimitive.Positioner>
    </MenuPrimitive.Portal>
  ...
  return (
    <MenuPrimitive.Item
      ...
      className={cn(
        ...
        // Base styles
        "gap-2 rounded-sm px-2 py-1.5 text-sm relative flex cursor-pointer items-center outline-hidden select-none",
        "data-disabled:pointer-events-none data-disabled:opacity-50 data-[inset]:pl-8",
        "[&_svg:not([class*='size-'])]:size-4 [&_svg]:pointer-events-none [&_svg]:shrink-0",
        // Default variant - focus/highlight states
        ...
        "dark:data-[variant=destructive]:focus:bg-destructive/20 dark:data-[variant=destructive]:data-highlighted:bg-destructive/20",
        // Icon color inheritance for destructive
        "data-[variant=destructive]:[&_svg]:text-destructive",
        // Non-destructive icon color on focus/highlight
...
function DropdownMenuSeparator({
  ...
  return (
    <MenuPrimitive.Separator
      ...
      className={cn("ui-dropdown-menu__separator", "bg-border -mx-1 my-1 h-px", className)}
      {...props}
    />
  )
}
