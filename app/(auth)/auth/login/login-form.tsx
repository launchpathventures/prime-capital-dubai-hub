...
interface LoginFormProps {
  mode: AuthMode
  redirectTo: string
  /** When true, show email/password form. When false (production), Google-only. */
...
export function LoginForm({ mode, redirectTo, showPasswordLogin = false }: LoginFormProps) {
  ...
  // Production supabase mode: Google-only (no email/password form)
  const googleOnly = mode === "supabase" && !showPasswordLogin
  ...
  return (
    <Stack gap="md">
      ...
      {/* Divider between Google and email/password (localhost only) */}
      {mode === "supabase" && showPasswordLogin && (
      ...
      {/* Email/password form — hidden in production supabase mode */}
      {!googleOnly && (
